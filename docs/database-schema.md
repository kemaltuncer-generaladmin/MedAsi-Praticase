...
| `learning_sentence` | text | Diğer Medasi ekosistemlerinin okuyacağı tek cümlelik özet |
...
#### `praticase.user_learning_history_sentences`
Kullanıcının PratiCase geçmişini diğer Medasi ürünlerinin okuyabileceği kısa
cümlelere çeviren security-invoker view. Örnek cümle:

```txt
Kullanıcı PratiCase teorik sınavında Anatomi dersi Solunum sistemi konusunda Larenks alt başlığındaki analysis düzeyi histological_anatomy tipindeki zor soruyu yanlış yaptı.
```

#### `praticase.praticase_learning_user_context(user_id, limit)`
Servis rolüyle diğer ekosistemler tarafından okunabilecek RPC. Dönen JSON:
